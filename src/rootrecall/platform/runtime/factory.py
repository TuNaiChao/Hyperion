# src/rootrecall/platform/runtime/factory.py
"""create_rootrecall_agent —— RootRecall lead agent 工厂(瘦身的中间件链 + create_agent)。
...
这是什么(面向小白):
  ...
  对标 deer-flow create_deerflow_agent,但 R3 大幅瘦身(F6):
    ...
    - **不自写** normalize_middleware_state_schema —— langchain create_agent 已自动合并
      ...
      在自己类上声明 state_schema,create_agent 自动并入图状态,无需改 RootRecallState 或本工厂。
...
from rootrecall.platform.runtime.middlewares.loop_detection import LoopDetectionMiddleware
from rootrecall.platform.runtime.middlewares.token_budget import TokenBudgetConfig, TokenBudgetMiddleware
from rootrecall.platform.runtime.middlewares.tool_output import ToolOutputBudgetConfig, ToolOutputBudgetMiddleware
from rootrecall.platform.runtime.middlewares.turn_budget import TurnBudgetConfig, TurnBudgetMiddleware
from rootrecall.platform.runtime.state import RootRecallState
...
class SummarizationConfig:
    ...
    - trigger_tokens:历史累计 token 估算 ≥ 此值就压(不是按消息条数——50 条可能才几千 token 白压一次,
      也可能已爆窗口;按真实 token 量才准)。用 `("tokens", N)` 而非 `("fraction",F)`:实测 RootRecall 的
...
def build_default_middlewares(
    ...
    *,
    ...
    链长上来时按这个顺序往本函数插(每条凭真实需求加,pull-by-need;见 runtime-middleware-policy 记忆):
       ...
       11. Memory(auto-inject)      (RootRecall 自建挂 MemoryService;**不抄** deer-flow MemoryMiddleware)
...
def create_rootrecall_agent(
    model: BaseChatModel,
    tools: Sequence[Any],
    *,
    system_prompt: str | None = None,
    middleware: Sequence[AgentMiddleware] | None = None,
    state_schema: type = RootRecallState,
    checkpointer: Any | None = None,
    name: str | None = None,
) -> CompiledStateGraph:
    """组装 RootRecall lead agent(深度调研那种长 agent 用它跑)。
    ...
    参数:
      ...
      state_schema  状态 schema,默认 RootRecallState。中间件自带的 state_schema 会被
