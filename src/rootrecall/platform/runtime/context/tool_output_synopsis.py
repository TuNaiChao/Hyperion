"""工具输出 synopsis(超长工具输出的结构化摘要)— deer-flow 逐字移植。

这是什么(面向小白):
  agent 调工具(grep/bash/read_file)时,一次返回几万行结果不能全塞进上下文(一塞就爆)。
  本模块把"超长输出"压成一张**结构化摘要**(纯函数、不调 LLM):按内容类型
  (json/csv/tsv/yaml/xml/code/text)抽 schema / 列名 / 符号 / 首尾片段。模型看到摘要 + 文件路径,
  需要细节时自己 read_file 取回。由 tool_output 中间件(同目录 ../middlewares/tool_output.py)
  在把大输出外化到磁盘后调用本模块生成摘要。

来源:deer-flow `agents/middlewares/tool_output_synopsis.py`(MIT,bytedance/deer-flow),
**逐字移植** —— 自包含纯函数(仅依赖 stdlib + pyyaml;defusedxml 可选,防 XML 实体展开 DoS)。
RootRecall 改动:仅本文件头注释;函数体保持一致,便于后续同步上游修复。设计见
docs/设计/runtime-harness-design.md §4.3。
...
def _try_table(content: str, *, delimiter: str, kind: Literal["csv", "tsv"]) -> ToolOutputSynopsis | None:
    ...
    if len(rows) > 1:
        for col_name, cell in list(zip(columns, rows[1], strict=False))[:_TABLE_COLUMN_LIMIT]:
