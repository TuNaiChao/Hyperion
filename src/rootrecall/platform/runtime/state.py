# src/rootrecall/platform/runtime/state.py
"""RootRecall agent 运行时状态 schema(对标 deer-flow ThreadState,瘦身起步)。

这是什么(面向小白):
  LangGraph 的 agent 跑起来时,要在各步骤间传一份"状态"(state)。这份 state 就是
  RootRecall 自己的 lead agent(深度调研那种长 agent)的工作台 —— 对话历史 + 摘要 + 委托账本。
...
class DelegationEntry(TypedDict):
    ...
    字段贴合 RootRecall 的 DelegateResult(tools/delegate.py):
...
class RootRecallState(AgentState):
    """RootRecall lead agent 的状态(R3 最小集,可扩展)。
    ...
    暂不引入(R3 范围,后续按需加,见模块末尾「扩展指南」):
      ...
      这些是 deer-flow 的字段,RootRecall 现阶段用不到,但 schema 任何时候都能加回来。
    """
...
# ──────────────────────────────────────────────────────────────────
# 扩展指南(面向后续阶段:怎么给 RootRecallState 加字段)
# ──────────────────────────────────────────────────────────────────
# 加字段有两扇门,按"谁拥有这字段"选,都不用改核心逻辑:
#
# ① 简单情况(某字段就 RootRecallState 自己用)→ 直接在本 class 加:
...
#   → 不动 RootRecallState,在**中间件类**上声明 state_schema:
...
#   能堆叠却互不干扰的关键,RootRecall factory(R3.0 后续展示)会照搬这个合并机制。
#
# 预留映射(到阶段):
