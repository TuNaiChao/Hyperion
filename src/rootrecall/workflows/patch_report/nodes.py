...
from rootrecall.workflows.patch_report.state import PatchReportState, PRFinding
...
async def node_fetch_prs(state: PatchReportState) -> dict:
    ...
    from rootrecall.services.patch.fetcher import fetcher_for_url
...
async def node_analyze(state: PatchReportState) -> dict:
    ...
    核心算法(cited-reporter + 风险/主题/安全分层)在 rootrecall.workflows.patch_report._analyze
    —— 窗口展示 · 用户手敲。单 PR 失败 → 降级一条 "(分析失败)" finding,不阻断批次。
    """
    from rootrecall.workflows.patch_report._analyze import _analyze_one_pr
...
def node_aggregate(state: PatchReportState) -> dict:
    ...
    from rootrecall.workflows.patch_report._aggregate import aggregate
...
def node_report(state: PatchReportState) -> dict:
    ...
    from rootrecall.workflows.patch_report.report import render_patch_report, verify_and_append
...
async def node_memorize(state: PatchReportState) -> dict:
    ...
    from rootrecall.services.memory.manager import get_memory_service
