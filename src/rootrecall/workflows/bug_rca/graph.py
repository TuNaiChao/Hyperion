...
⚠️ **post-pivot(2026-08-06)参考实现,不再是主路径。** RootRecall 已从「调度型 orchestrator」转向
「tool+skill server / 领域 harness」(见 docs/设计/harness-pivot-design.md):bug-RCA 的主路径改成
**opencode + bug-rca skill(.claude/skills/bug-rca/SKILL.md)+ 6 个 RootRecall MCP 工具**,agent 自己驱动、
...
CLI `rootrecall bug-rca` 也保留(向后兼容,加了 deprecate 提示)。**不要**把本 workflow 暴露成 MCP 工具。

---
六节点线性流水线(踩坑 #2,2026-07-31:砍 RootRecall 侧定位漏斗 —— 旧 recall/localize/
...
证伪式自审产出,执行硬门控由 RootRecall validate_patch(非 LLM)。收敛靠每 delegate call 的 steps +
单 schema + max-loop 兜底(不重蹈 glm-5.2 单 loop 97K 不收敛)。
opencode 定位/修复时经 MCP 调 RootRecall 工具(search_codebase/recall/filter_logs,见 bug-rca-design.md §6)。
...
async def run(repo_root: str, trigger: str | None, log_path: str | None = None) -> dict:
    ...
    # 后者尤其要绝对 —— MCP server 的 cwd 是 workspace(≠ RootRecall 根),相对 log_path 会找不到。
    initial: BugRcaState = {
