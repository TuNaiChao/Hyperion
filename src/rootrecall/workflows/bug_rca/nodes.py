...
六步(踩坑 #2,2026-07-31:砍 RootRecall 侧定位漏斗 —— 旧 recall/localize/assemble_localize 三节点
...
recall_lessons —— 但它只翻记忆预注入先验、不定位,和被砍的旧 recall 本质不同,不算重造漏斗):
  ingest → recall_lessons  (1.5 确定性预注入:翻记忆 top-K 同类 bug 教训 → 预进 localize prompt,②[b])
        → delegate_localize_loop  (阶段① 定位:opencode 自定位 + 调 rootrecall_* 工具,verify-refine 循环,max K1 轮,同会话)
...
R3.1 #54-rework(B):弃「多候选采样 + majority voting」改「迭代 verify-refine」——
  ...
  - 执行硬门控(RootRecall 侧,非 LLM):validate_patch Tier0 apply/revert(#50 repro 落地后加强);
...
2026-07-31:rerank/majority_voting 整体移除(无 oracle 平凡白烧);RootRecall 侧定位漏斗砍掉改工具驱动
(opencode 经 MCP 调 search_codebase/recall;日志切片用 grep/awk,见 bug-rca-design.md §6)。
2026-08-10:`rootrecall_filter_logs` 工具撤销(deer-flow/omp 双证:专门日志切片工具没必要,opencode 的
...
from rootrecall.services.code_index.loc_translate import line_wrap_content, merge_intervals
...
from rootrecall.tools.delegate import CodingAgentDelegate
from rootrecall.workflows.bug_rca.state import BugRcaState
...
async def node_recall_lessons(state: BugRcaState) -> dict:
    ...
    面向小白:外勤(opencode)出发定位前,RootRecall 先翻一遍"历史案卷"(长期记忆),把和这个 bug
    可能相关的历史教训**确定性地**塞进它的任务单开头(不用外勤自己想起去查)。这是"0 决策轮"的
    廉价预取 —— 对标 deer-flow DynamicContextMiddleware(hybrid:预取 + 工具)。

    为什么不算"重造漏斗"(踩坑 #2):漏斗是 RootRecall 自己**定位**(file→function→line)再喂,和
    ...
    → 返空段(绝不阻断 workflow;delegate 还能调 MCP `rootrecall_recall` 工具按需深挖)。
    """
    ...
    try:
        ...
        # 主动调 rootrecall_recall(走 recall,会 bump)才算"真命中"。一行可切回 recall。
...
async def node_recall_for_repair(state: BugRcaState) -> dict:
    ...
    面向小白:阶段① 外勤读完了日志、搞清了"出啥错"(problem_summary),RootRecall 这时再翻一遍历史案卷
    ...
    都没有 / scope 缺 → 返空(绝不阻断;delegate 还能调 rootrecall_recall MCP 工具按需深挖)。
...
def _build_localize_prompt(trigger: str, schema: dict, log_path: str | None = None,
    ...
    **预喂「历史同类 bug 教训」**(②[b],hybrid 的廉价预取那段:0 决策 turn 先验,opencode 仍
    ...
    if prior_lessons:
        prior_block = (
            "### 历史同类 bug 教训(RootRecall 记忆先验)###\n"
            "下面是 RootRecall 从历史 bug 分析中沉淀的、可能与本次相关的教训。"
...
{log_hint}
### 你的任务 ###
1. 自主定位根因(为什么出 bug):优先用 RootRecall 提供的差异化工具(语义检索代码 / 翻历史同类 bug
...
async def node_delegate_localize_loop(state: BugRcaState) -> dict:
    ...
    for i in range(k1):
        ...
        last_result = await delegate.run(
            ...
            agent="rootrecall-localize", continue_session=i > 0,
        )
...
def node_assemble_repair(state: BugRcaState) -> dict:
    ...
    if repair_prior:
        prior_block = (
            "### 历史同类 bug 的修法(RootRecall 记忆先验)###\n"
            "下面是 RootRecall 从历史 bug 分析中沉淀的、可能与本次相关的教训与修法。"
...
1. 基于上面的根因,**用 edit 工具直接修改 `./code/` 里根因涉及的文件**来修这个 bug。
   **不要把 unified diff 贴在回复里** —— RootRecall 会用 `git diff` 观察你对 code/ 的实际改动生成补丁(行号/格式天然正确)。
2. 只改根因相关文件,禁止顺手重构。
...
async def node_delegate_repair_loop(state: BugRcaState) -> dict:
    ...
    for i in range(k2):
        ...
        last_result = await delegate.run(
            prompt, cwd=code_dir, output_schema=schema, timeout=None,
            agent="rootrecall-repair", continue_session=True,
        )
