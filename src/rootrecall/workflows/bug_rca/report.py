...
from rootrecall.workflows.bug_rca.nodes import _coerce_evidence_line
...
def render_report(state: BugRcaState) -> str:
    ...
    parts.append("### 生成溯源\n")
    parts.append(
        "- 本报告由 RootRecall bug-RCA workflow 生成:opencode 自主定位+修复"
        "(调 RootRecall MCP 工具 search_codebase / recall / filter_logs)+ `git diff` 观察补丁 + "
