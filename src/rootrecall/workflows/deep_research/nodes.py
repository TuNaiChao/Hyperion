...
from rootrecall.workflows.deep_research.report import render_report
from rootrecall.workflows.deep_research.state import DeepResearchState, ModuleFinding
...
def node_index(state: DeepResearchState) -> dict:
    ...
    复用 build_index(rootrecall index 同款)+ CodeGraph.build。两者都慢(全仓解析),各跑一次。
    """
...
async def node_research(state: DeepResearchState) -> dict:
    ...
    在 rootrecall.workflows.deep_research._research —— 窗口展示 · 用户手敲。
    """
    from rootrecall.workflows.deep_research._research import _research_modules
