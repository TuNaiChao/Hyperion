...
这一层干什么(面向小白)
------------------------
RootRecall 是"接案调度员",它把 bug-RCA 里"读代码 / 写补丁 / 给根因"的力气活
...
RootRecall 的差异化在记忆 + 调度,委托 = 把通用能力外包(三锁定决策 #2)。
...
# RootRecall 根:解析 config 相对路径(config/opencode_rootrecall.json),给 env OPENCODE_CONFIG 用。
# delegate.py 在 src/rootrecall/tools/,parents[3] = RootRecall 根。
_ROOTRECALL_ROOT = Path(__file__).resolve().parents[3]
...
class DelegateResult:
    ...
    tool_calls: list[str] = field(default_factory=list)  # opencode 调过的工具名(含 rootrecall_* MCP;observability)
...
class CodingAgentDelegate(abc.ABC):
    ...
    async def run(
        ...
        *,
        ...
        agent: str | None = None,  # 指定 delegate agent(rootrecall-localize/repair)
    ...
    def from_config(cls) -> CodingAgentDelegate:
        ...
        short = {
            "opencode": "rootrecall.tools.delegate:OpencodeDelegate",
            "omp": "rootrecall.tools.delegate:OmpDelegate",
            "claude": "rootrecall.tools.delegate:ClaudeDelegate",
        }
...
class OpencodeDelegate(CodingAgentDelegate):
    ...
    async def _run_once(self, prompt, cwd, output_schema, cfg, timeout, agent, continue_session, *,
        ...
        # env OPENCODE_CONFIG:注入 RootRecall 自带 opencode 配置(agent+steps+permission+mcp,config/opencode_rootrecall.json),
        # 与用户全局 opencode.json(provider/key)合并(opencode 配置 8 层合并,路 1 调研)。
        env = dict(os.environ)
        if cfg.config:
            _cp = Path(cfg.config)
            _cp = _cp if _cp.is_absolute() else _ROOTRECALL_ROOT / _cp
            env["OPENCODE_CONFIG"] = str(_cp)
        # ROOTRECALL_CODEBASE:告诉 `rootrecall mcp serve`(opencode 经 MCP 拉起的子进程)查哪个代码库的
        ...
        if "__" in _parent:
            env["ROOTRECALL_CODEBASE"] = _parent.split("__", 1)[0]
        ...
        # 审计 opencode 调了哪些工具(§6.2;含 rootrecall_* MCP 工具 → 验证「工具驱动委托」真生效)。
    ...
    def _delegate_log_dir(cwd) -> Path:
        ...
        if ws_log.parent.exists():  # workspace 根有 delegate/ 目录才往那写
            return ws_log
        return _ROOTRECALL_ROOT / "data" / "runtime" / "delegate_log"
