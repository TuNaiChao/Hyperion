...
干什么(面向小白)
  把"外面的文档"吃进 RootRecall 的长期记忆,变成可检索、带溯源、能持续学习的知识项。三类入口:
...
为什么不自造存储(调研定论,非凭感觉)
  mem0/cognee/Zep/Letta 2026 格局 + mnemopi veracity-consolidation 逐条核实:RootRecall 已走的结构化
  KnowledgeItem 路线 = mnemopi 的 Memory 式(bayesian 合并 / source_tier 权重 / 确定性 sha256 ID /
  bi-temporal + superseded_by 全就位)。Cognee 的 Extract→Cognify→Load 是从零造整条管线;RootRecall
...
from rootrecall.services.memory.backends.native.extract import extract_items  # noqa: F401 (re-export 便于外部单测)
from rootrecall.services.memory.manager import MemoryService, get_memory_service
from rootrecall.services.memory.schema import Evidence, KnowledgeItem, Scope, SourceTier, make_id  # noqa: F401 (Evidence 供 PatchIngestPipeline.run 核心用)
from rootrecall.services.trigger_parser.parser import parse_issue  # 复用现成 loader(.md/.txt/.pdf)
