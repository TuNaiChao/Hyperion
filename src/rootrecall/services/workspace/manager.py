...
# patch 由 RootRecall 用 git diff 观察 code/ 改动生成(行号/格式天然对)。
_AGENTS_MD = """\
...
- **直接用 edit 工具改 `./code/` 里的文件来修 bug**。**不要把 unified diff 贴在回复里** ——
  RootRecall 会用 `git diff` 观察你对 code/ 的实际改动来生成补丁,这样行号/格式天然正确。
...
def create_workspace(
    ...
    git_env = {
        **os.environ,
        "GIT_AUTHOR_NAME": "rootrecall",
        "GIT_AUTHOR_EMAIL": "rootrecall@local",
        "GIT_COMMITTER_NAME": "rootrecall",
        "GIT_COMMITTER_EMAIL": "rootrecall@local",
    }
