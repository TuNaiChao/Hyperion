...
**给系统软件(C 代码库,如 wpa_supplicant / bluez)做「带记忆的 bug 根因定位 + 深度调研」的领域 harness —— 记忆 + 代码情报 + 日志取证 + 补丁验证 + 标准流程 skill,作为 MCP tool/skill server 供 opencode(主)/ codex / claude code 调用。** 不自己调度 coding agent 跑固定管线;重活(读码 / 改代码)归成熟 coding agent,RootRecall 当后勤(递手术刀 + 菜谱 + 记忆库)。

## 能做什么

三大场景,共享一个 harness(同一个 MCP server + 记忆 + 代码情报),各配一本 skill(菜谱):

- **bug 根因定位** ✅ —— opencode + `bug-rca` skill + 7 个 RootRecall 工具,agent 自驱定位根因、改代码、验证补丁、落盘补丁、沉淀教训(不走固定管线,能自纠)。
...
uv run rootrecall index <仓库路径> <仓库名>
...
**接上你的 agent:**
- **opencode**(主):配置 [config/opencode_rootrecall.json](config/opencode_rootrecall.json)(注册 rootrecall MCP + `rootrecall*` 工具放行 + `rootrecall-bug-rca` 等 agent);skill 放 [.claude/skills/](.claude/skills/),opencode 自动发现。
...
[ skills: bug-rca ✅ | patch-review ⏳ | research ⏳ ]
   ────────────────→  RootRecall MCP server(rootrecall mcp serve)
                  ...
                  code_index + CRG(代码情报)· MemoryService(记忆,recall 4 路)· workspace(补丁验证)
                          ↓
                  rootrecall CLI(基建:index / mcp serve / memory / research)
```

一个 MCP server、N 个 skill、N+M 个工具,所有 agent 都能用。RootRecall 不抢 agent 的活,只递工具 + 菜谱。

## 七个 MCP 工具(共享,已落)

| 工具 | 作用 |
|---|---|
| `rootrecall-memory_recall` | 翻长期记忆(历史 bug 教训 / 代码事实),带 file:line 溯源 |
| `rootrecall-search_codebase` | 语义 + 符号检索,**只回索引里真实存在的符号**(防幻觉) |
...
| `rootrecall-export_report` | 把分析报告落盘成 `data/bug_rca/<repo>-rca.md`(交付硬门;空内容报错) |
| `rootrecall-memory_memorize` | 写记忆 / 沉淀教训 |
...
post-pivot 地基已落(commit `47654bd`,已 push):D0 MCP 双 transport + 8 工具 + `bug-rca` skill + `rootrecall-bug-rca` opencode agent(validate/export_patch/memorize/export_report 硬门,e2e 绿)+ 老 orchestrator 降级留参考。**下一步**:补丁分析(1a)→ 批量报告(1b)→ 调用链(2a)→ 跨版本(2b)。详见 [docs/设计/harness-v2/README.md](docs/设计/harness-v2/README.md) 路线表。
