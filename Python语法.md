...
**给每个模块写一份"专属调研任务书"**——把结构图(CRG)用机器名(如 `community-3`)圈出来的几团代码,变成带**人话名字 + 针对性问题清单**的调研计划,这样后面派出去的子 agent 一上来就知道"这个模块我重点该问啥",而不是对着同一句通用模板泛泛地读。

**打个比方:派侦察兵探陌生大楼**

想象代码仓库是一栋**你从没进过的超大办公楼**,你要摸清它怎么运作。

- **第 2 步 index(CRG)**:先用无人机扫一遍,把大楼**自动分区**——这堆工位算 A 区、那堆算 B 区。但无人机只会起机器名:`zone-3`、`zone-7`。你拿到一张分区图,却不知道每区是干嘛的。
- **R3.2 的老做法**:给每个区派一个侦察兵,塞同一句**通用指令**:"去看看这区是干啥的、有啥对外接口。" → 每个人问的问题一模一样,回来写的报告都差不多,重点不突出。
- `_plan.py`(R3.3.1 新做法): 派兵之前,先让一个参谋看一眼每区的花名册(成员文件 + 核心人物/hub 符号),给每区写一份专属任务书:
  - 区 3 的花名册全是 `p2p_*` 开头、一堆握手函数 → 参谋命名「**P2P 设备发现与协商**」,任务书写:"查它的公开接口、主握手流程;**因为处理外来输入,顺便盯一眼输入校验(安全视角)**。"
  - 区 7 全是 `driver_nl80211_*`、跟内核交互 → 命名「**驱动适配层(nl80211)**」,任务书换一组问题:"查它跟内核的交互边界、资源生命周期。"

**每个侦察兵拿到的问题都不一样,还都戳中了那区真正的特点。**
...
2. R2 最值钱的教训(7 次端到端 + 调研迭代得出)：

**glm-5.2 在 opencode 单 agent loop 发散**(97K token 全工具,最后 prose 不收敛)→ 正解是 **Agentless 式分阶段 + opencode `steps` 强制收尾**:

- 数据铁证:Agentless 分阶段 32%/$0.70 vs SWE-agent 单 loop 18.3%/$2.53(又便宜又稳)
- 消融:skeleton(小上下文)> 整文件(lost-in-the-middle)= glm-5.2 内联大片过载根因
- 解药:多阶段(任务单一)+ `steps` 限轮(强制总结)+ `--continue`(不重复探索)

R2 这一程从「单 delegate 发散不收敛」摸到「分阶段+steps+session」的正解,中间穿插了 workspace 设计、方式 B 实验、多阶段调研、A+C 评估 —— 虽然坎坷,但**把委托 opencode 的最佳实践彻底摸清了**(分阶段>单 loop、steps 强制收敛、workspaceChanges>LLM吐diff),这些都是 R3 及以后的生产级地基。
