...
|---|---|---|---|---|
| "为什么 X 会断/泄漏/死锁/崩",查 bug 根因 + 修复 | **bug-rca** | 多假设清单 + 证伪迭代定位根因,出补丁 + 报告 | ✅ 改 | 用户验证后 |
| "这个补丁/PR 干啥 / 能不能打上 / 该不该合 / 有没有副作用" | **patch-review** | 鉴定一个补丁:做了什么、能否 apply、影响面、合入建议 | ❌ 只读 | 用户验证后 |
| "上游这些 commit 哪些该合进来 / 哪些已修过 / 会不会冲突"(同一个 git 仓) | **upstream-merge** | 逐 commit 三态判定(已修/建议合/冲突)+ 相关性 + 决策表 | ❌ 只评估 | 用户验证后 |
| "v25 修了这个 bug、v20 还没修,帮我改 v20"(两个独立发行版线) | **backport** | 读 v25 fix → 语义判 v20 有无同一 bug → 适配改 v20 + 验 apply | ✅ 改 | 用户验证后 |
