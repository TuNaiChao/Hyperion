...
**R3.3.2 = P3 Verifier 硬化** ✅ P3.1 已实现(2026-08-04:逐符号@行 + Existence@Line Ratio,commit 2e5747b;P3.1.1 ±行容差 near 档;P3.2 LSP fallback / P3.3 打分块 pull-by-need)。原现状:`_verify.py:31-38` 只查 `exists()`,不验 symbol@line;但 cited-reporter 把结构化 citations 存 state `findings[].citations[]`{file,line,symbol,claim},parse_file API 子 agent 已在用 `_research.py:155`):
