...
- `bug-rca/SKILL.md` ✅ —— 7 步:recall→search→filter→假设+证伪→blast→改+validate(硬门)→memorize(硬门)。
...
- `hyperion-bug-rca` ✅ —— steps=25,edit/bash/hyperion* allow,validate+memorize 硬门。
...
**共享工具(所有用例复用,6 个已落 + build_check 待落):**
