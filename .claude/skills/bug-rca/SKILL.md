---
name: bug-rca
description: 在 C/系统软件仓库(Linux 内核、BlueZ、wpa_supplicant、systemd、dbus、网络栈)定位 bug 根因并修复。用户让你查 bug/崩溃/挂起/回归/CVE 的根因、问"为什么 X 会断/泄漏/死锁"、或修这类 bug 时用。你负责推理和改代码;Hyperion 工具提供记忆、代码检索、日志切片、影响面、补丁 apply 校验、补丁/报告落盘。补丁只有在干净 apply 且经人/真机验证后才算正确——在此之前持续迭代。
allowed-tools:
...
- **假设 ↔ 证伪循环**:用 `memory_recall` / `search_codebase` / `filter_logs` 取证,每轮主动证伪当前根因;经住证伪才定论。
...
| `hyperion_filter_logs(path, since, until)` | 大日志按时间窗切片 | **从比现象更早切**(根因在上游);若根因假设落在窗起点,把 `since` 前推重筛 |
...
- 未经验证不 `memorize`。
- `filter_logs`:根因在现象上游——从更早切;看到窗口边界提醒就前推 `since`。
