...
**★ 第四决策(2026-07-29 修正):runtime harness 自建。** Hyperion 自己要有 agent 运行时上下文管理(对标 deer-flow 中间件链 + OpenHands 3 层 Condenser→View→ConversationMemory),能跑长 agent(深度调研 R3 必需,读几千文件上下文必爆);**coding 能力仍委托 opencode/omp** 不变。边界:coding 动作→委托;运行时(压历史/token 预算/截工具输出/并行子任务/断点续跑)→自建。设计 runtime-harness-design.md(本机归档 docs-bak/设计/runtime-harness-design.md) + 对标 deer-flow-runtime-参考.md(本机归档 docs-bak/调研/deer-flow-runtime-参考.md)。排期:**R3 开场搭骨架**(factory+TokenBudget+ToolOutputBudget+HyperionState+checkpointer;2026-07-30 从 R2末挪 R3:R2 九步不依赖 runtime、不验,R3 深度调研边搭边验)→ R3 中上场(summarization+loop+subagents)→ R5 补齐。详见 backlog #45。

设计文档:**architecture.md(本机归档 docs-bak/设计/architecture.md)**(v2 总纲)+ memory-design.md(本机归档 docs-bak/设计/memory-design.md) / bug-rca-design.md(本机归档 docs-bak/设计/bug-rca-design.md) / deep-research-design.md(本机归档 docs-bak/设计/deep-research-design.md) + p1-code-understanding-design.md(本机归档 docs-bak/设计/p1-code-understanding-design.md)(code_index 已成层)。计划文件:`~/.claude/plans/crystalline-dazzling-ladybug.md`。
